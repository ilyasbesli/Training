	...
	public int solution(int[] A) {
		...
		for (int i = 0; i < A.length; i++) {
			...
			if (countMap.get(currentValue) == null)
				countMap.put(currentValue, Integer.valueOf(1));
			else
				countMap.put(currentValue, countMap.get(currentValue) + 1);
			// control count of dominant value
			if (countMap.get(currentValue) > (A.length / 2))
				return i;
		}
		return -1;
	}
